...
def _probe(video: Path) -> Optional[Tuple[int, int, float, bool]]:
    """Return (width, height, fps, has_audio) for the source clip.
...
def append_outro(
    ...
    if not probed:
        ...
        return str(video_path)
    width, height, fps, has_audio = probed
    ...
    # The card is scaled and fps-matched explicitly. concat demands identical
    # dimensions, pixel format and frame rate across inputs, and silently
    # produces a broken file when they differ.
    chain = (
        f"[1:v]scale={width}:{height},fps={fps},format=yuv420p,setsar=1[card];"
        f"[0:v]scale={width}:{height},fps={fps},format=yuv420p,setsar=1[main];"
    ...
    logger.info(f"Outro: appended {seconds}s card for {brand} -> {dest.name}")
    return str(dest)
